...
func (r *Reader) CreateColumnReader(path ...string) (*ColumnReader, error) {
	node, err := r.meta.Schema.ResolveNodeByPath(path)
	if err != nil {
		return nil, err
	}
	if node.PhysicalType == nil {
		return nil, fmt.Errorf("node is not a leaf (path=%s)", path)
	}

	column, err := r.meta.Schema.ResolveLeafColumn(path)
	if err != nil {
		return nil, err
	}
...
func (r *ColumnReader) NextBatch() (bool, error) {
	for _, rowGroup := range r.meta.RowGroups {
		err := parseRowGroupData(rowGroup, r.meta.Schema, r.r)
		if err != nil {
			return false, err
		}
	}
